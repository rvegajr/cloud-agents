...
Work the phases in order, A through H. Each phase has the same four parts:
...
Slack app, and a bot you run on your laptop. E makes it survive a closed lid. F,
G, and H are optional and independent of each other.
...
flowchart LR
  ...
  GH -->|merge to branch| Vercel[Vercel project]
  GH -->|merge to branch| RailwayApp[Railway project]
  Vercel -->|"Vercel for Slack"| Slack["#project-fixbot"]
  RailwayApp -->|"Railway webhook (Slack muxer)"| Slack
  Bot -->|"deploy cmd (VERCEL_TOKEN / RAILWAY_TOKEN)"| Vercel
  Bot -->|deploy cmd| RailwayApp
...
- **Your host runs the bot.** One long-lived Node process holding a websocket to
  ...
  says why.
- **Vercel and Railway host the apps** the agent writes code for. Their deploy
  notifications (phase G) come from their own Slack integrations, not from the
  bot. The bot's `deploy` command (phase F) is the *manual* rebuild, a different
  thing that happens to land in the same channel.
...
| GitHub PAT | `GITHUB_TOKEN` | github.com/settings/tokens → **classic** | `repo` | the bot, to flip a draft PR to ready | `.env`, host variables | H |
...
| Slack incoming webhook | — | same app → Incoming Webhooks | `incoming-webhook`, one URL per channel | **Railway**, to post deploy cards | Railway project settings | G |
| Vercel for Slack link | — | vercel.com/integrations/slack/new + `/vercel signin` | Vercel account ↔ Slack user | `@Vercel`, to post deploy events | Vercel + Slack | G |
| Vercel API token | `VERCEL_TOKEN` | vercel.com/account/tokens | full account, or scope to one team | the bot, to *watch* a deploy it fired | `.env`, host variables | F |
| Vercel team id | `VERCEL_TEAM_ID` | team settings | — (identifier, not a secret) | same | `.env`, host variables | F, team projects only |
| Vercel deploy hook | in `SLACK_DEPLOYS` | project → Settings → Git → Deploy Hooks | one hook per branch; URL *is* the credential | the bot, to fire a deploy | `.env`, host variables | F |
| Railway project token | `RAILWAY_TOKEN` | target project → Settings → Tokens | one project, one environment | the bot, to deploy that env | `.env`, host variables | F |
| Railway account token | `RAILWAY_API_TOKEN` | account settings → Tokens | every project you can see | the bot, if you refuse per-project tokens | `.env`, host variables | F, prefer the project token |
...
| Jam PAT | `JAM_TOKEN` | jam.dev → Settings → MCP | `mcp:read` (add `mcp:write` only to post comments) | the bot, to read recordings | `.env`, host variables | H |
...
- **`RAILWAY_TOKEN` and `RAILWAY_API_TOKEN` are different tokens with different
  HTTP headers**, and the kit uses the same names the Railway CLI does. A project
  token goes in `Project-Access-Token`; an account token goes in `Authorization:
  Bearer`. See `credentialsFromEnv` in
  [src/lib/slack-deploy.ts](src/lib/slack-deploy.ts). Prefer the project token,
  scoped to the one environment Slack is allowed to ship.
...
run on Vercel, even though the apps it deploys do. Vercel Functions are
...
railway ssh                     # then: cat version.json, env, etc.
```
...
**Verify:** `npm run doctor -- --phase E`, then `railway logs` showing the same
Socket Mode banner you saw locally, then one mention from your phone.
...
## Phase F: deploy from a Slack channel (optional)

Condenses [ARTICLE-SLACK.md](ARTICLE-SLACK.md) step 6½. This is the **manual**
rebuild — `@<bot> api deploy`. Auto-deploy status is phase G and involves none of
these tokens.

**Preconditions:** phase D green.

**Do:** add targets to `.env` as `<project>[/<env>]=<provider>:<spec>`:

```
SLACK_DEPLOYS=api=vercel:https://api.vercel.com/v1/integrations/deploy/prj_xxx/yyy,web/uat=railway:<projectId>/uat/web+api,web/production=railway:<projectId>/production/web+api
SLACK_DEPLOYERS=U0123ABCD,U0456EFGH
VERCEL_TOKEN=...
VERCEL_TEAM_ID=team_...
RAILWAY_TOKEN=...
```

An entry with no `/<env>` is the default target (`@<bot> api deploy`); the others
need `env=<name>`. Without `VERCEL_TOKEN` the hook still fires but the bot cannot
report READY or FAILED. Without a Railway token, Railway deploys fail outright.
An empty `SLACK_DEPLOYERS` means anyone in an allowed channel can ship.

**Verify:** `npm run doctor -- --phase F`. It validates the shape of every deploy
hook URL, resolves every Railway project/environment/service name through the
API, and checks that each `U…` in `SLACK_DEPLOYERS` is a real member. It does
**not** fire a hook — proving a deploy works means running one on purpose.

**Human gate:** minting the Vercel deploy hook and the Railway project token, and
deciding who is on `SLACK_DEPLOYERS`.

---

## Phase G: auto-deploy notifications (optional)
...
`SLACK_DEPLOYS`, no tokens in `.env`.** The doctor cannot verify any of it from
outside, so the check for this phase is a printed checklist and the real proof is
a merge.
...
**Verify:** `npm run doctor -- --phase G` prints this checklist and reports
whether the target repos' hosts are even known to it. The real verification is a
merge that produces a card in the channel.

---

## Phase H: Jam evidence and ready-for-review (optional)
...
**Verify:** `npm run doctor -- --phase H`. It confirms the Jam CLI and token, and
...
| Slack incoming webhook URL | Railway project settings | the repo; it is a bearer credential in URL form |
| `GITHUB_TOKEN`, `VERCEL_TOKEN`, `RAILWAY_TOKEN`, `JAM_TOKEN` | gitignored `.env`, host variable store | the repo, Slack, the agent's prompt |
...
1. Prefer a Railway **project** token scoped to one environment over an account
   token that can reach every project you own.
2. Set `SLACK_ALLOWED_CHANNELS` and `SLACK_DEPLOYERS`. Empty means "anyone,
   anywhere the bot was invited."
...
- [ ] A reply in that same thread resumed the same agent rather than starting a
      new one.
- [ ] The bot survives a redeploy: `railway logs` shows the Socket Mode banner
      after `railway up`.
- [ ] (F) `@<bot> <project> deploy` reported a terminal status in the thread.
- [ ] (G) A merge produced a deploy card in the channel from `@Vercel` or the
      Railway webhook.
- [ ] (H) A verified PR came out of draft on its own.
- [ ] `git status` is clean and `git ls-files` does not include `.env`.
