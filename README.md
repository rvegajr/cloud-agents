...
```
   brief  ─>  prompt  ─>  agent on a VM  ─>  verification  ─>  artifact (PR)  ─>  feedback
     ^                                                                              |
     └──────────────────────────────────────────────────────────────────────────────┘
```
...
| **Context** (paths, patterns to copy) | Twenty minutes of exploration in a fresh clone, or inventing a second convention |
...
- **Trigger from events, not a terminal.** Wrap `src/02-cloud-oneshot.ts` in a
  GitHub Action on `issues.labeled` and pass the issue body as the brief. Or a
  webhook from Linear. The kit's exit codes are already CI-friendly.
...
cloud-agents/
  ...
  .env.example                   credentials + target repo
  briefs/
  ...
  src/
    ...
    03-cloud-pipeline.ts         plan -> implement -> verify on one agent
