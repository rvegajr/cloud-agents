...
1. **Page checks as tests the Solver writes.** Both R1 reviews said the same thing independently: a done-check that only a browser can attest is attested by a narrated walk, and that is weaker evidence than the check asks for. The Verifier also costs the most of anything in the loop (26 to 37 minutes a batch on the local model, with frontier fallbacks). A Solver that writes the page check as an automated test at Devise — a Playwright spec where the repo has `@playwright/test`, a Node test against a fake window where it does not — turns the check into a command the loop runs itself. The prompt rule is written, and so is the scaffold (2026-09-20 evening): when only a real browser can attest a D and the repo has no `@playwright/test`, Devise adds one unit that installs it, writes `playwright.config.js` with `webServer` on the bar's `start`, and fetches Chromium; each page unit's Check is then `npx playwright test <spec>`, and plan-lint refuses a Playwright check with nothing to run it. Measured 2026-09-21 by rerunning `examples/corpus/05-packing-list.md`: Devise scaffolded Playwright in U1 and wrote five specs that U5's Check ran green in the Hand's gate, and look back still walked the same eight prose Ds in four batches, the same as the corpus run, because the done-checks in `PROBLEM.md` had not changed and they are what the Verifier walks. The other half, built the same day: Devise may rewrite a D's Check to the command that runs a test it wrote this turn, and nothing else in `PROBLEM.md`; the loop checks exactly that and then runs the D at look back. Two more things the rerun taught: Playwright's `test-results/` appeared when a Check ran and was read as the Hand's change (now excluded in `.git/info/exclude` at run start), and a unit that curls the page must depend on the unit that writes it (the Hand asked, the re-plan fixed it). The rerun itself finished as a product: ten of ten done-checks met, the reviewer found the same arrow-key focus bug it found on the corpus build (twice out of two, a candidate for the oracle), the repair unit passed its gate, and the reviewer's own check then failed only because its script was written under `mktemp`'s directory where `require("playwright")` cannot see the repo's `node_modules`; run from the repo it passes. Same rule, now in the review prompt too. To be measured again on this code.
