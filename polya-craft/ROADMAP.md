...
- **Docker here is Colima, which shares only the home directory, twice.** First D7 of the json-count run: the fresh clone under the system temp root mounted as an empty folder. Then D10 of `ledger-report` (corpus 06): the check mounted a file it had made with `mktemp`, which lands under `/var/folders`, and the container saw an empty directory; the Solver reproduced it with busybox and called the check wrong, correctly. `WORK_ROOT=$HOME/.cache/cloud-agents-work` and `TMPDIR=$WORK_ROOT/tmp` together; the corpus driver sets both. Not a code defect, and written down because it bit twice in one day.
